...
<form action = {{route('titulo.update', ['titulo'=>$titulo->id])}} method="post">
...
<select name="pessoa_id">
  ...
  @foreach ( $pessoas as $pessoa )
       <option value="{{$pessoa->id}}" {{($titulo->pessoa_id ?? old('pessoa_id'))==$pessoa->id ? 'selected':''}}>"{{$pessoa->nome ?? old('nome')}}"</option>
  @endforeach 
</select>
...
<select name="categoria_id">
  ...
  @foreach ( $categorias as $categoria )
       <option value="{{$categoria->id}}" {{($titulo->categoria_id ?? old('categoria_id'))== $categoria->id ?'selected':''}}>"{{$categoria->desc ?? old('desc')}}"</option>
  @endforeach 
</select>
